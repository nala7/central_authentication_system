...
%Los tokens son una construcción arbitraria generada por el sistema que confirma la identidad de quien dice ser [15] La autenticación basada en tokens implica el intercambio de credenciales de autenticación del cliente por un token de autenticación generado por el servidor, de modo que para las solicitudes posteriores de los clientes para acceder a los recursos de un servicio, los tokens se envían como parte de la solicitud en el encabezado HTTP al servidor. Esta reutilización del mismo token de acceso de usuario para acceder a recursos protegidos regidos por ciertas políticas puede ser un desafío, especialmente cuando se actualiza una política de acceso a recursos y el token de acceso de usuario sigue siendo válido. De hecho, esto puede introducir una vulnerabilidad basada en el tiempo (ataque de tiempo) en los recursos protegidos. En el caso de múltiples usuarios accediendo al recurso, el índice de vulnerabilidad puede aumentar exponencialmente. Por lo tanto, se ha propuesto y evaluado un modelo de autenticación y autorización que limita dichas vulnerabilidades y mejora el acceso seguro a los recursos. [16]
...
\input{MainMatter/Tables/AutenticacionVsAutorizacion.tex}
